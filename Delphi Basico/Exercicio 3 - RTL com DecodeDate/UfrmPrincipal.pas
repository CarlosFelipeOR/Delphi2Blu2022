...
begin
  if not TryStrToDate(edtEntrada.text, xEntrada) then
    raise Exception.Create('Erro: Insira uma data válida');
...
begin
  ...
  lbResultado.Caption :='Hoje é dia ' + IntToStr(xDia)
                       + ' de ' + xMesExtenso
                       + ' de ' + IntToStr(xAno);
...
begin
  if not TryStrToInt(edtEntrada.text, xEntrada) then
    raise Exception.Create('Erro: Insira um número inteiro');
...
begin
  if not TryStrToFloat(edtEntrada.text, xEntrada) then
    raise Exception.Create('Erro: Insira um número decimal');
