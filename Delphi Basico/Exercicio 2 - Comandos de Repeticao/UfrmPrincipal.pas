...
begin
  ...
  case TComandos(rdgComandos.ItemIndex) of
    cmdFor:
      begin
        xResultado := ResultFor(xNumero1, xNumero2); //Chama Função FOR
      end;

    cmdWhile:
      begin
        xResultado := ResultWhile(xNumero1, xNumero2); //Chama função WHILE
      end;

    cmdRepeat:
      begin
        xResultado := ResultRepeat(xNumero1, xNumero2); //Chama Função REPEAT
      end;
  end;
