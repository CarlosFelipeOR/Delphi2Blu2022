...
begin
  ...
  xContNovo := 0;
  repeat
    xModelo := inputbox( 'Cálculo de Desconto','Modelo','');
    xAno := StrToInt(inputbox( 'Cálculo de Desconto','Ano',''));
    xValorSemDesconto :=StrToInt(inputbox( 'Cálculo de Desconto',
                                           'Valor Sem Desconto',''));
    if xAno <= 2000 then
      begin
    ...
    xContinuar :=UpperCase(inputbox('Cálculo de Desconto',
                        'Calcular desconto de mais um carro? S(Sim) ou N(Não)',
                        ''));
  until (xContinuar = 'N');
  mmValoresCalculados.lines.Add('Total de Carros Calculados : ');
  mmValoresCalculados.lines.Add('Até 2000: ' + IntToStr(xContVelho));
